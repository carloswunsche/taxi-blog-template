...
function Icon({wanted, title='', styleClass='ion-icon'}) {
    let html = '';
    // Start with svg tag
    ...
    // Accesibility stuff
    html += `aria-labelledby="${title}" role="img" height="16" width="16" viewBox="0 0 512 512">`;
    // Title tag (must have one based on W3C specification)
    html += `<title id="${title}">${title}</title>`;

    // Add path tag based on wanted icon (and close svg tag also)
    switch (wanted) {    
        case 'book-outline':
            html += `<path d="M256 160c16-63.16 76.43-95.41 208-96a15.94 15.94 0 0116 16v288a16 16 0 01-16 16c-128 0-177.45 25.81-208 64-30.37-38-80-64-208-64-9.88 0-16-8.05-16-17.93V80a15.94 15.94 0 0116-16c131.57.59 192 32.84 208 96zM256 160v288" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="32"/></svg>`;
            break;
        case 'caret-back-outline':
            html += `<path d="M321.94 98L158.82 237.78a24 24 0 000 36.44L321.94 414c15.57 13.34 39.62 2.28 39.62-18.22v-279.6c0-20.5-24.05-31.56-39.62-18.18z"/></svg>`;
            break;
        case 'caret-forward-outline':
            html += `<path d="M190.06 414l163.12-139.78a24 24 0 000-36.44L190.06 98c-15.57-13.34-39.62-2.28-39.62 18.22v279.6c0 20.5 24.05 31.56 39.62 18.18z"/></svg>`;
            break;
        case 'bicycle':
        ...
        case 'locate':
            ...
            break;
        case 'code-slash':
            html += `<path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="32" d="M160 368L32 256l128-112M352 368l128-112-128-112M304 96l-96 320"/></svg>`;
            break;
        case 'speedometer':
